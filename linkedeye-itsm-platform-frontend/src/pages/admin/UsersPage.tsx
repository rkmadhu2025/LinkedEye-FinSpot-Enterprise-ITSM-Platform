      ...
      <div className="space-y-10 reveal-stagger min-h-screen -m-6 p-6" style={{ background: isDark ? 'linear-gradient(180deg, #030712 0%, #0a0f1a 100%)' : '#f9fafb' }}>
         ...
         <Modal isOpen={isCreateModalOpen} onClose={() => { setIsCreateModalOpen(false); resetNewUserForm(); }} title="INITIALIZE IDENTITY ONBOARDING" size="md">
            <div className="space-y-6 pt-4">
               ...
               <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Comms Channel (Phone)</label>
                  <input type="tel" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:ring-1 focus:ring-indigo-500 font-mono" placeholder="+91..." value={newUser.phone} onChange={e => setNewUser({ ...newUser, phone: e.target.value })} />
               </div>
               <div>
         ...
         <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="MODIFY IDENTITY BIOMETRICS" size="md">
            {editingUser && (
               <div className="space-y-6 pt-4">
                  ...
                  <div>
                     <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Comms Protocol (Phone)</label>
                     <input type="tel" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white font-mono" value={editingUser.phone || ''} onChange={e => setEditingUser({ ...editingUser, phone: e.target.value })} />
                  </div>
                  <div className="flex justify-end gap-3 pt-6 border-t border-white/5">
