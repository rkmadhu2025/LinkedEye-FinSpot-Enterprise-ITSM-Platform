...
function transformIncident( backendIncident: BackendIncident ): Incident {
  ...
  const assignee = backendIncident.assignee ? {
    id: backendIncident.assignee.id,
    email: backendIncident.assignee.email,
    firstName: backendIncident.assignee.firstName,
    lastName: backendIncident.assignee.lastName,
    role: backendIncident.assignee.role,
  } : undefined;

  // Transform assigned group if present (backend returns string, create minimal Group object)
  const assignedGroup = backendIncident.assigned_group ? {
    id: backendIncident.assigned_group, // Use group name as ID since we don't have actual ID
    name: backendIncident.assigned_group,
    code: backendIncident.assigned_group,
    groupType: 'support' as const,
    isActive: true,
  } : undefined;
  ...
  return {
    ...
    assignedGroup: assignedGroup as any,
